...
1)}else{self[keyName](fn,true);self._listeners[name].push(fn)}}};this.generateGetterSetter=function(name){self[this.nameConversion(name)]=function(){var args=Array.prototype.slice.call(arguments);if(args.length==0)return self._data[name];else{var value=args.shift(),deep=args.shift();return set(name,value,deep)}}};function fireOnKey(key){self._listeners[key]=self._listeners[key]||[];for(var i in self._listeners[key])if(self._listeners[key][i]){if(debug)G.log("--Event Listener on key: "+key+" Fired--");
self._listeners[key][i](self._data[key])}}function set(key,value,deep){self._listeners[key]=self._listeners[key]||[];if(deep&&typeof value==="object"&&value!=null){var empty=value instanceof Array?[]:{};self._data[key]=G.deepCopy(empty,value,true)}else self._data[key]=value;fireOnKey(key);return self._data[key]}}});G.provide("browser",{ieVersion:function(){var rv=-1;if(navigator.appName=="Microsoft Internet Explorer"){var ua=navigator.userAgent;var re=new RegExp("MSIE ([0-9]{1,}[.0-9]{0,})");if(re.exec(ua)!=
