...
"object"&&source[key]!=null){target[key]=source[key]instanceof Array?[]:{};G.deepCopy(target[key],source[key],overwrite,transform)}return target},noConflict:function(altName){if(altName=="G")window.G=G;else if(altName){window[altName]=G;window.G=_G}globalName=altName;return G}};window.G=G;var globalName="G";G.provide("Tracker",{iframe:null,endpoint:"http://analytics.groupit.com",accountToken:null,init:function(){G.Tracker.iframe=G.ApiClient.createHiddenIframe()},trackEvent:function(category,action,
...
G.newContribution,json)}});G.provide("models.groupit",{objectName:"groupit",path:"/groupits",keys:["id","product_image","product_uri","product","price","message","surprise","quantity","user_id","recipient","options","active","lead_uri","support_uri","purchased","groupit_type","created_at","updated_at","hash_digest","organizer","amount_raised"],index:function(config){var params={include:["user","participants","contributions"]};G.DataObject.commonIndex(config,G.models.groupit,G.newGroupit,params,wrapIncludes);
function wrapIncludes(models){for(var i in models){var j;var model=models[i];if(model.user)model.user(G.newUser(model.user()));if(model.participants){var parts=[];for(j in model.participants()){var participant=model.participants()[j];participant.user=G.newUser(participant.user);parts.push(G.newParticipant(participant))}model.participants(parts)}if(model.contributions){var contrs=[];for(j in model.contributions()){var contribution=model.contributions()[j];var user=commonUser(contribution.user_id,model.participants());
contribution.user=user;contrs.push(G.newContribution(model.contributions()[j]))}model.contributions(contrs)}}}function commonUser(userId,participants){for(var i in participants){var p=participants[i];if(p.userId()==userId)return p.user()}return null}},Base:function(){}});G.provide("",{newUser:function(json){var namespace=G.models.user;return G.DataObject.commonConstructor(namespace,G.newContribution,json)}});G.provide("models.user",{objectName:"user",path:"/users",keys:["name","login","organized_before",
